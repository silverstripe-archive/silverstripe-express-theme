<% loop Menu(1) %>
	<li class="$LinkingMode  <% if $LinkingMode = current %>active<% end_if %> <% if MenuChildren %> dropdown <% end_if %>">
		<a href="$Link" title="Go to the $Title.XML page" class="$LinkingMode <% if MenuChildren %> dropdown-toggle" data-toggle="dropdown"<% else %>"<% end_if %><% if AccessKey %> accesskey="$AccessKey"<% end_if %>>
